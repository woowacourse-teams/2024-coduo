...
public class TodoController implements TodoDocs {
    ...
    public void createTodo(@PathVariable("accessCode") final String accessCode, @RequestBody @Valid final CreateTodoRequest request) {
        todoService.createTodo(accessCode, request.content());
    }

    @PatchMapping("/todos/{todoId}/contents")
    public void updateContent(@PathVariable("todoId") final long todoId,
    ...
    public void updateTodoOrder(@PathVariable("todoId") final long todoId, @RequestBody UpdateTodoOrderRequest request) {
